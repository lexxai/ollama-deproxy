...
### Docker from container registry 'ghcr.io/lexxai/ollama-deproxy'

A Dockerized proxy/wrapper utility for Ollama, available via GitHub Container Registry (GHCR) at `ghcr.io/lexxai/ollama-deproxy`.

#### Getting Started

##### 1. Prepare Environment Variables

Before running the container, create and configure your environment file:

```bash
cp .env.example .env
```

Open the newly created `.env` file and adjust the configuration values to match your setup.

#### Usage Modes

##### Run as a Webserver

To spin up the container as an interactive web server listening on the standard Ollama port:

```bash
docker run -it --rm  --env-file=.env -p 11434:11434 ghcr.io/lexxai/ollama-deproxy
```

##### Run as a CLI Tool

If you want to bypass the webserver and interact directly with the underlying CLI tool, override the entrypoint.

**Using default `.env` configurations:**

```bash
docker run -it --rm  --env-file=.env -p 11434:11434 --entrypoint ollama-deproxy ghcr.io/lexxai/ollama-deproxy
```

**Passing arguments (e.g., viewing the help menu):**

```bash
docker run -it --rm  --env-file=.env -p 11434:11434 --entrypoint ollama-deproxy ghcr.io/lexxai/ollama-deproxy --help
```
